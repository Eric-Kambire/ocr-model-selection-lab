...
def import_qlicker_clients(
    ...
    *,
    ...
    """Télécharge les deux faces et le label de chaque client dans le contrat local.

    Chaque résultat décrit un client indépendamment. Une erreur réseau ou une
    réponse non conforme ne bloque jamais les autres identifiants demandés.
    """
    ...
    for raw_client_id in client_ids:
        ...
        try:
            ...
            (client_dir / f"{client_id}.json").write_text(json.dumps(label, ensure_ascii=False, indent=2), encoding="utf-8")
            item.update({"recto_source": str(recto), "verso_source": str(verso), "label_path": str(client_dir / f"{client_id}.json")})
...
def _request_url(config: QlickerImportConfig, path_template: str, client_id: str) -> str:
    """Construit une URL avec {client_id}; les paramètres non déclarés échouent vite."""
    path = path_template.format(client_id=client_id)
