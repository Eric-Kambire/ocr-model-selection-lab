...
# Ce module contient les détails numériques. Le registre public et les
# adaptateurs par méthode vivent dans des fichiers séparés.
...
from .diagnostics import GENERATE_INTERMEDIATE_STEPS, is_required_stage
...
def _save_stage(
    ...
    *,
    ...
    if not GENERATE_INTERMEDIATE_STEPS.get() and not is_required_stage(name):
        return ""
    cv2, _ = _opencv()
    index = len(stages) + 1
