...
def crop_cni_document(source_path: Path, output_path: Path, *, debug_path: Path | None = None) -> dict[str, Any]:
    ...
    return {"image_path": str(output_path), "crop_status": "crop_detected_perspective", "crop_box": _points_box(ordered), "coverage": round(float(selected["area"]) / max(1, original.shape[0] * original.shape[1] * scale * scale), 4), "score": round(float(selected["score"]), 4), "ratio": round(float(selected["ratio"]), 4), "rectangularity": round(float(selected["rectangularity"]), 4), "source_sent_unchanged": False, "edge_trim": list(trim), "debug_image_path": str(debug_path) if debug_path and debug_path.is_file() else None}
