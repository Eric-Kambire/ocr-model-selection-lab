...
from scripts.cni_crop_stepper_app import _mm_to_px, build_robust_pipeline
...
def test_simulation_a4_creates_pdf_metrics_and_reusable_log(tmp_path: Path):
    ...
    # L'interface utilise désormais directement la pipeline hybride V4.
    result = build_robust_pipeline(str(card), "simulate_a4", 150, 7.5, 120)
    state = result[0]
    ...
    assert report["metrics"][5]["step"] == "6. Image envoyée au modèle"
